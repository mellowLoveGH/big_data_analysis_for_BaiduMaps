...
# split log into 4 parts: notice, query, info, result
def segments(ln):
    L = len(ln)
    if L<=100:
        return False, []
...
# get the cuid, store it as dic/dictionary
...
# process query, split is as key-value
...
# another faster way to process query, get useful keys and store them as key-value in dic
...
# ensure GPS coordinate belongs to which city
...
# record the PV/UV and success rate indicators
...
# process info, split it as key-value
...
# another faster way to process info, store useful keys in dic
...
# check in-out flag: indoor or outdoor
...
import time
# time checking for GPS and wifi
...
# filter GPS: remove low-quality GPS
...
# split map as grid, by default: 10*10
# 10m * 10m grid
...
# calculate the distance by longitude and latitude
...
# get position and convert to float
def find_pos(dic3, pos_name):
    if pos_name in dic3:
        return float(dic3[pos_name])
    return ""

# get positions: GPS, cell, wifi, prop, fp
...
# for accuracy, categorize as <30, <50, <100, <500, <1000, >1000
def gradient_dis(d):
    if d<=30:
        return 30
    elif d<=50:
...
# calculate the distance between cell, wifi, prop, fp and GPS 
...
# store data for every mapper, to speed up
...
for data in sys.stdin:
    ...
    try:
        ...
        dic_data.append("1PV")
        mc = mc + 1
        # for high-accuracy indoor localization
        if "&indoor_flag=1" in data:
            global_dic["T1"] += 1
        elif "&indoor_flag=2" in data:
            global_dic["T2"] += 1
        #
        f, ss = segments(data)
        if f:
            ...
            # UV
            user_counter(dic1, dic_data)
            # ---------------- GPS
            #gps = find_gps(dic3, dic_data)
            gps_ff01 = filter_gps01(dic2, dic3)
            #
            # NLP, open_platform, map
            gps = nlp_map_op(dic2, dic3, 0, dic_data, gps_ff01)
            # cl, cellflag
            find_cl_cellflag(dic2, dic3, dic_data)
            # wifi, wififlag, propflag, fpflag, clsflag
            wf_f = find_wf(dic2, dic_data)
            if wf_f == 1:
                find_wifi_prop_fp_cls(dic3, dic_data) 
            # inoutflag
            find_inoutflag(dic3, dic_data)
            #
        ...
        if mc % 5000 == 0:
            user_set = print_user_gps(user_set)
